...
def search_action():
    ...
    if not search_text:
        ...
        if len(search_items) >= 2:
            ...
            for text in rearch_link:
                title.append(get_vn_article_title(text))
            if english_content:
                result_text_content.delete(1.0, tk.END)
                for title, link in zip(title, rearch_link):
                    formatted_text = f"{title}\n{link}\n\n"
                    result_text_content.insert(tk.END, formatted_text)
                    result_text_content.tag_add("hyperlink", f"{result_text_content.index(tk.END)} - {len(link) + 1} chars", tk.END)
                    result_text_content.tag_configure("hyperlink", foreground="blue", underline=True)
                    result_text_content.tag_bind("hyperlink", "<Button-1>", lambda event, link=link: webbrowser.open(link))
                result_text_links.delete(1.0, tk.END)
